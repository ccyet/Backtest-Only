...
def get_direction_options(entry_factor: str) -> list[str]:
    return [label for label, _ in ENTRY_DIRECTION_OPTIONS.get(entry_factor, ENTRY_DIRECTION_OPTIONS["gap"])]
...
def format_detail_for_display(detail_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for column in DETAIL_NAV_COLUMNS:
        if column in display_df.columns:
            display_df[column] = display_df[column].map(lambda value: format_number(value, 4))
    ...
    if "win_flag" in display_df.columns:
        display_df["win_flag"] = display_df["win_flag"].map(
            lambda value: "是" if pd.notna(value) and int(value) == 1 else ("否" if pd.notna(value) else "")
        )
...
def format_scan_for_display(scan_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for column in display_df.columns:
        ...
        elif column in {"scan_id", "signal_count", "closed_trade_candidates", "executed_trades"}:
            display_df[column] = display_df[column].map(
                lambda value: f"{int(value):,}" if pd.notna(value) else ""
            )
        elif column in {
...
summary_cols[0].metric(
    "交易方向",
    "做多"
    if direction_label_to_internal(current_entry_factor, current_direction_label) == "up"
    else "做空",
)
...
if submitted:
    ...
    params = AnalysisParams(
        ...
        stock_codes=normalize_stock_codes(stock_scope_text),
        gap_direction=direction_label_to_internal(str(entry_factor), str(direction_label)),
